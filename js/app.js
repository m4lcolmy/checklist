...
function buildItemEl(section, item, idx){
  ...
  if(item.critical){
    ...
    confirmLabel.className = "item-confirm-label";
    confirmLabel.textContent = "Onayla?";
    bodyEl.appendChild(confirmLabel);
  }
...
function toggleItem(itemId){
  ...
  var li = state.itemEls[itemId];
  if(pair.item.critical && !check.ok && li.dataset.pending !== "true"){
    li.dataset.pending = "true";
    return;
  }
  li.dataset.pending = "false";
  check.ok = !check.ok;
  check.ts = nowTs();
...
function onSearch(q){
  ...
  if(state.searchQuery){
    ...
    clear.hidden = true;
    Object.keys(state.itemEls).forEach(function(k){
      state.itemEls[k].style.display = "block";
    });
  }
}
...
function bindGlobalEvents(){
  ...
  if(searchToggle && searchWrap){
    searchToggle.addEventListener("click", function(){
      ...
      if(!searchWrap.hidden) searchInp.focus();
